...
static t_class *dkscldeg_class;

typedef struct _dkscldeg {
	t_object x_obj;
	char scalename[10];
	int curscale[10];
	t_float offset;
...
static void dkscldeg_float(t_dkscldeg *x, t_float f){
	int returnval;
	int iptoctave;
	int input = (int) f;
	int curscale[10];
...
static void dkscldeg_pickscale(t_dkscldeg *x, t_symbol *s){
...
static void *dkscldeg_new(t_symbol *s, t_floatarg f){
	t_dkscldeg *x = (t_dkscldeg *)pd_new(dkscldeg_class);
	dkscldeg_pickscale(x, s);
	x->offset = f;
...
void dkscldeg_setup(void){
	dkscldeg_class = class_new(gensym("dkscldeg"), (t_newmethod)dkscldeg_new, 0,
			sizeof(t_dkscldeg), 0, A_DEFSYMBOL, A_DEFFLOAT, 0);
	class_addfloat(dkscldeg_class, (t_method)dkscldeg_float);
	class_addmethod(dkscldeg_class, (t_method)dkscldeg_pickscale, gensym("scale"), A_SYMBOL, 0);
}
