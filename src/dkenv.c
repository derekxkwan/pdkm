...
void *dkenv_tilde_new(t_symbol *s, int argc, t_atom *argv){
	int i;
	t_dkenv_tilde *x = (t_dkenv_tilde *)pd_new(dkenv_tilde_class);
	t_atom params[10];
	x->x_sr = sys_getsr();
        x->x_sz = ENVSZ;
        x->x_type = PERC;
...
static t_int *dkenv_tilde_perform(t_int *w){
	 t_dkenv_tilde *x = (t_dkenv_tilde *)(w[1]);
	...
	for(i=0; i<n; i++){
           ...
           if(x->x_act){
                ...
                output = dkgetlin(x->x_env, x->x_sz, phs);
                output *= x->x_lvl;
...
static void dkenv_tilde_dsp(t_dkenv_tilde *x, t_signal **sp){
	...
	// divide by tablelen to map to 0 to 1 range,..freq/samplerate
	    dsp_add(dkenv_tilde_perform, 3, x, sp[0]->s_vec, sp[0]->s_n);
		int sr = sp[0]->s_sr;
                if(sr != x->x_sr){
