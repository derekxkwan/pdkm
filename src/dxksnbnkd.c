...
static t_class *dxksnbnkd_tilde_class;

typedef struct _dxksnbnkd_tilde_tilde {
	t_object x_obj;
	...
	t_inlet *x_rlistlet;
	t_outlet *x_outlet;
} t_dxksnbnkd_tilde_tilde;

static void dxksnbnkd_tilde_abs_to_rel(t_dxksnbnkd_tilde_tilde *x){
...
static double get_t60mult(t_dxksnbnkd_tilde_tilde *x, float ringms){
...
static void dxksnbnkd_tilde_setmode(t_dxksnbnkd_tilde_tilde*x, t_symbol *s){
...
static void dxksnbnkd_tilde_init(t_dxksnbnkd_tilde_tilde *x){
	int i;
...
static void dxksnbnkd_tilde_setrampsamp(t_dxksnbnkd_tilde_tilde *x, t_float rampms){
	int sr = x->x_sr;
...
static void dxksnbnkd_tilde_freqs(t_dxksnbnkd_tilde_tilde *x, t_symbol *s, int argc, t_atom *argv){
	int i;
	for(i=0; i<argc; i++){
		if(i<DXKSBMAX){
	...
	x->x_numsin = argc;
	dxksnbnkd_tilde_abs_to_rel(x);
}

static void dxksnbnkd_tilde_amps(t_dxksnbnkd_tilde_tilde *x, t_symbol *s, int argc, t_atom *argv){
	int i;
	for(i=0; i<argc; i++){
		if(i<DXKSBMAX){
...
static void dxksnbnkd_tilde_rings(t_dxksnbnkd_tilde_tilde *x, t_symbol *s, int argc, t_atom *argv){
	int i;
	for(i=0; i<argc; i++){
		if(i<DXKSBMAX){
...
static void *dxksnbnkd_tilde_new(t_symbol *s, int argc, t_atom *argv){
	int i;
	t_dxksnbnkd_tilde_tilde *x = (t_dxksnbnkd_tilde_tilde *)pd_new(dxksnbnkd_tilde_class);
	x->x_mode = DXKSBMODE;
	int sincount = 0; //count actual number of sines
	x->x_sr = sys_getsr(); //get sampling rate
	dxksnbnkd_tilde_setrampsamp(x, DXKSBRAMPMS);
	x->x_curfreq = 0.f;
	x->x_pendfreq = 0.f;
			...
			if(argc >= 2){//should be at leas two args
				...
				if(strcmp(curarg->s_name, "-mode") == 0){
					if(arg2 != &s_){//if not num, else just ignore arg
						dxksnbnkd_tilde_setmode(x, arg2);
					}; 
					argc-=2;
					argv+=2;
				}
					...
					if(arg2 == &s_){
						...
						dxksnbnkd_tilde_setrampsamp(x, rampms);
						argc -=2;
						argv += 2;
					}
					else{//not a number
...
static void dxksnbnkd_tilde_arrs_reset(t_dxksnbnkd_tilde_tilde *x){
...
static void dxksnbnkd_tilde_rampfunc(t_dxksnbnkd_tilde_tilde *x){
...
static double dxksnbnkd_tilde_getsin(t_dxksnbnkd_tilde_tilde *x, double phs){
...
static t_int *dxksnbnkd_tilde_perform(t_int *w){
	 t_dxksnbnkd_tilde_tilde *x = (t_dxksnbnkd_tilde_tilde *)(w[1]);
	...
	while(n--){
		...
		if(freq > 0.f && numsin > 0){
			for(i=0; i<numsin; i++){
				...
				double curout = dxksnbnkd_tilde_getsin(x, phs);
				curout *= (curamp);
...
static void dxksnbnkd_tilde_dsp(t_dxksnbnkd_tilde_tilde *x, t_signal **sp){
	//(freq*tablelen)/(samplerate) = array values per sample to advance
	// divide by tablelen to map to 0 to 1 range,..freq/samplerate
	    dsp_add(dxksnbnkd_tilde_perform, 3, x, sp[0]->s_vec, sp[0]->s_n);
		int sr = sp[0]->s_sr;
		if(x->x_sr != sr){
			int i, numsin = x->x_numsin;
			x->x_sr = sr;
...
static void dxksnbnkd_tilde_float(t_dxksnbnkd_tilde_tilde *x, t_float freq){
	x->x_pendfreq = freq;
	x->x_newatt = 1;
}

static void dxksnbnkd_tilde_bang(t_dxksnbnkd_tilde_tilde *x){
	x->x_pendfreq = x->x_curfreq;
	x->x_newatt = 1;

}

static void *dxksnbnkd_tilde_free(t_dxksnbnkd_tilde_tilde *x){
...
void dxksnbnkd_tilde_setup(void){
	dxksnbnkd_tilde_class = class_new(gensym("dxksnbnkd~"), (t_newmethod)dxksnbnkd_tilde_new, 0,
			sizeof(t_dxksnbnkd_tilde_tilde), CLASS_DEFAULT, A_GIMME, 0);
	class_addfloat(dxksnbnkd_tilde_class, (t_method)dxksnbnkd_tilde_float);
	class_addbang(dxksnbnkd_tilde_class, (t_method)dxksnbnkd_tilde_bang);
	class_addmethod(dxksnbnkd_tilde_class, (t_method)dxksnbnkd_tilde_dsp, gensym("dsp"), A_CANT, 0);
   class_addmethod(dxksnbnkd_tilde_class, (t_method)dxksnbnkd_tilde_freqs, gensym("freqs"), A_GIMME, 0);
   class_addmethod(dxksnbnkd_tilde_class, (t_method)dxksnbnkd_tilde_amps, gensym("amps"), A_GIMME, 0);
   class_addmethod(dxksnbnkd_tilde_class, (t_method)dxksnbnkd_tilde_rings, gensym("rings"), A_GIMME, 0);
   class_addmethod(dxksnbnkd_tilde_class, (t_method)dxksnbnkd_tilde_setmode, gensym("mode"), A_SYMBOL, 0);
   class_addmethod(dxksnbnkd_tilde_class, (t_method)dxksnbnkd_tilde_setrampsamp, gensym("ramp"), A_FLOAT, 0);
