...
static t_class *dxkgrnrd_tilde_class;

typedef struct _dxkgrnrd_tilde
{
    t_object x_obj;
	//tabread stuff
    int x_npoints;
...
static double getreadscale(t_dxkgrnrd_tilde *x, float transpose){
...
static void freegr(t_dxkgrnrd_tilde *x, int actidx){
...
static int getnextgr(t_dxkgrnrd_tilde *x){
	int i;
...
static void dxkgrnrd_tilde_grainrsmp(t_dxkgrnrd_tilde *x, t_float grms){
...
static void dxkgrnrd_tilde_overlap(t_dxkgrnrd_tilde *x, t_float overlap){
	int overlapint;
	if(overlap <= 1.){
		overlapint = 1;
		}
...
static void dxkgrnrd_tilde_randsize(t_dxkgrnrd_tilde *x, t_float randsize){
	int randsizeint;
	if(randsize <= 0.){
		randsizeint = 0;
		}
...
static void dxkgrnrd_tilde_randsizeamt(t_dxkgrnrd_tilde *x, t_float randsizeamt){
	if(randsizeamt <= 0.){
		randsizeamt = 0.;
		};
...
static void dxkgrnrd_tilde_randamp(t_dxkgrnrd_tilde *x, t_float randamp){
	int randampint;
	if(randamp <= 0.){
		randampint = 0;
		}
...
static void dxkgrnrd_tilde_randampamt(t_dxkgrnrd_tilde *x, t_float randampamt){
	if(randampamt <= 0.){
		randampamt = 0.;
		};
...
static void dxkgrnrd_tilde_randtp(t_dxkgrnrd_tilde *x, t_float randtp){
	int randtpint;
	if(randtp <= 0.){
		randtpint = 0;
		}
...
static void dxkgrnrd_tilde_randtpamt(t_dxkgrnrd_tilde *x, t_float randtpamt){
	if(randtpamt <= 0.){
		randtpamt = 0.;
		};
...
static void dxkgrnrd_tilde_posvar(t_dxkgrnrd_tilde *x, t_float posvar){
	if(posvar <= 0.){
		posvar = 0.;
		};
	x->x_posvar = posvar;
...
static void dxkgrnrd_tilde_hopvar(t_dxkgrnrd_tilde *x, t_float hopvar){
	if(hopvar <= 0.){
		hopvar = 0.;
		};
	x->x_hopvar = hopvar;
}
static void dxkgrnrd_tilde_makeenv(t_dxkgrnrd_tilde *x, int envtype){
		int i;
...
static void dxkgrnrd_tilde_envtype(t_dxkgrnrd_tilde *x, t_symbol *env){
	int envtype;
	...
	if(envtype != x->x_env && x->x_init == 1){
		dxkgrnrd_tilde_makeenv(x, envtype);
	};
	x->x_env = envtype; //store what type it is in struct
}


static void *dxkgrnrd_tilde_new(t_symbol *s, int argc, t_atom *argv)
{
	int i;
    t_dxkgrnrd_tilde *x = (t_dxkgrnrd_tilde *)pd_new(dxkgrnrd_tilde_class);
	x->x_init=0;
	if(argc == 0){
		goto errstate;
	}
		...
		while(argc > 0){
			...
			if(strcmp(arg0->s_name, "-grainrate") == 0){
				if((argv+1) -> a_type == A_FLOAT){
					...
					dxkgrnrd_tilde_grainrsmp(x, grms);
					argc-= 2;
					argv+= 2;
				}
				else{
				...
				if((argv+1) -> a_type == A_FLOAT){
					...
					dxkgrnrd_tilde_overlap(x, overlap);
					argc-= 2;
					argv+= 2;
				}
				else{
				...
				if((argv+1) -> a_type == A_FLOAT){
					...
					dxkgrnrd_tilde_randsize(x, randsize);
					argc-= 2;
					argv+= 2;
				}
				else{
				...
				if((argv+1) -> a_type == A_FLOAT){
					...
					dxkgrnrd_tilde_randsizeamt(x, randsizeamt);
					argc-= 2;
					argv+= 2;
				}
				else{
				...
				if((argv+1) -> a_type == A_FLOAT){
					...
					dxkgrnrd_tilde_randamp(x, randamp);
					argc-= 2;
					argv+= 2;
				}
				else{
				...
				if((argv+1) -> a_type == A_FLOAT){
					...
					dxkgrnrd_tilde_randampamt(x, randampamt);
					argc-= 2;
					argv+= 2;
				}
				else{
				...
				if((argv+1) -> a_type == A_FLOAT){
					...
					dxkgrnrd_tilde_randtp(x, randtp);
					argc-= 2;
					argv+= 2;
				}
				else{
				...
				if((argv+1) -> a_type == A_FLOAT){
					...
					dxkgrnrd_tilde_randtpamt(x, randtpamt);
					argc-= 2;
					argv+= 2;
				}
				else{
				...
				if((argv+1) -> a_type == A_FLOAT){
					...
					dxkgrnrd_tilde_posvar(x, posvar);
					argc-= 2;
					argv+= 2;
				}
				else{
				...
				if((argv+1) -> a_type == A_FLOAT){
					...
					dxkgrnrd_tilde_hopvar(x, hopvar);
					argc-= 2;
					argv+= 2;
				}
				else{
		...
		dxkgrnrd_tilde_makeenv(x, x->x_env);
		x->x_init = 1;
    return (x);
	};
	errstate:
...
static t_int *dxkgrnrd_tilde_perform(t_int *w)
{
    t_dxkgrnrd_tilde *x = (t_dxkgrnrd_tilde *)(w[1]);
...
static void dxkgrnrd_tilde_set(t_dxkgrnrd_tilde *x, t_symbol *s)
{
    t_garray *a;
...
static void dxkgrnrd_tilde_dsp(t_dxkgrnrd_tilde *x, t_signal **sp)
{
    dxkgrnrd_tilde_set(x, x->x_arrayname);
	x->x_nextgr = 0;
	
    dsp_add(dxkgrnrd_tilde_perform, 4, x,
        sp[0]->s_vec, sp[1]->s_vec, sp[0]->s_n);

	float newsr = sp[0]->s_sr * .001f;
	if(x->m_sr != newsr){
		x->m_sr = newsr; //sample rate in ms
		dxkgrnrd_tilde_grainrsmp(x, x->x_grainrms);
	};

}

static void *dxkgrnrd_tilde_free(t_dxkgrnrd_tilde *x)
{
	outlet_free(x->x_outlet);
	return (void *)x;
}

void dxkgrnrd_tilde_setup(void)
{
    dxkgrnrd_tilde_class = class_new(gensym("dxkgrnrd~"),
        (t_newmethod)dxkgrnrd_tilde_new, (t_method)dxkgrnrd_tilde_free,
        sizeof(t_dxkgrnrd_tilde), CLASS_DEFAULT, A_GIMME, 0);
    CLASS_MAINSIGNALIN(dxkgrnrd_tilde_class, t_dxkgrnrd_tilde, x_f);
    class_addmethod(dxkgrnrd_tilde_class, (t_method)dxkgrnrd_tilde_dsp,
        gensym("dsp"), A_CANT, 0);
    class_addmethod(dxkgrnrd_tilde_class, (t_method)dxkgrnrd_tilde_set,
        gensym("set"), A_SYMBOL, 0);
    class_addmethod(dxkgrnrd_tilde_class, (t_method)dxkgrnrd_tilde_overlap,
        gensym("overlap"), A_FLOAT, 0);
    class_addmethod(dxkgrnrd_tilde_class, (t_method)dxkgrnrd_tilde_randsize,
        gensym("randsize"), A_FLOAT, 0);
    class_addmethod(dxkgrnrd_tilde_class, (t_method)dxkgrnrd_tilde_randsizeamt,
        gensym("randsizeamt"), A_FLOAT, 0);
    class_addmethod(dxkgrnrd_tilde_class, (t_method)dxkgrnrd_tilde_randtp,
        gensym("randtp"), A_FLOAT, 0);
    class_addmethod(dxkgrnrd_tilde_class, (t_method)dxkgrnrd_tilde_randtpamt,
        gensym("randtpamt"), A_FLOAT, 0);
    class_addmethod(dxkgrnrd_tilde_class, (t_method)dxkgrnrd_tilde_randamp,
        gensym("randamp"), A_FLOAT, 0);
    class_addmethod(dxkgrnrd_tilde_class, (t_method)dxkgrnrd_tilde_randampamt,
        gensym("randampamt"), A_FLOAT, 0);
    class_addmethod(dxkgrnrd_tilde_class, (t_method)dxkgrnrd_tilde_grainrsmp,
        gensym("grainrate"), A_FLOAT, 0);
    class_addmethod(dxkgrnrd_tilde_class, (t_method)dxkgrnrd_tilde_posvar,
        gensym("posvar"), A_FLOAT, 0);
    class_addmethod(dxkgrnrd_tilde_class, (t_method)dxkgrnrd_tilde_hopvar,
        gensym("hopvar"), A_FLOAT, 0);
    class_addmethod(dxkgrnrd_tilde_class, (t_method)dxkgrnrd_tilde_envtype,
