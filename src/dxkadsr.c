...
#ifndef DXKADSRL
#define DXKADSRL 1.f //default level
#endif
...
static t_class *dxkadsr_tilde_class;

typedef struct _dxkadsr_tilde {
	t_object x_obj;
...
static double ms_to_inc(t_dxkadsr_tilde *x, float mstime){
	double srms = x->x_sr * 0.001f;
	double inc = 1.f/((double)mstime*(double)srms);
	return inc;
}

static void dxkadsr_tilde_adsrer(t_dxkadsr_tilde *x){
	int stage = x->x_stage;
...
static t_int *dxkadsr_tilde_perform(t_int *w){
...
static void dxkadsr_tilde_float(t_dxkadsr_tilde *x, t_float input){
	if(input <= 0.f){
		x->x_newrel = 1;
	}
	else{
...
static void dxkadsr_tilde_attack(t_dxkadsr_tilde *x, t_float attms){
...
static void dxkadsr_tilde_decay(t_dxkadsr_tilde *x, t_float decms){
...
static void dxkadsr_tilde_release(t_dxkadsr_tilde *x, t_float relms){
...
static void dxkadsr_tilde_sustain(t_dxkadsr_tilde *x, t_float sustain){
	...
	if(sustain != sus){
		...
		x->x_sus = sustain;
		dxkadsr_tilde_release(x, relms);
		dxkadsr_tilde_decay(x, decms);
	};
}

void dxkadsr_tilde_level(t_dxkadsr_tilde *x, t_float level){
	if(level < 0.f){
		level= 0.f;
	};
	x->x_lvl = level;
...
void *dxkadsr_tilde_free(t_dxkadsr_tilde *x){
	inlet_free(x->x_alet);
	inlet_free(x->x_dlet);
	inlet_free(x->x_slet);
	inlet_free(x->x_rlet);
...
static void *dxkadsr_tilde_new(t_symbol *s, int argc, t_atom *argv){
	
	t_dxkadsr_tilde *x = (t_dxkadsr_tilde *)pd_new(dxkadsr_tilde_class);
	...
	switch(argc){
		...
		setting:
			...
			x->x_sus = sus;
			dxkadsr_tilde_attack(x,att);
			dxkadsr_tilde_decay(x,dec);
			dxkadsr_tilde_release(x,rel);
	};
...
errstate:
		pd_error(x, "dxkadsr~: improper args");
		return NULL;
}


static void dxkadsr_tilde_dsp(t_dxkadsr_tilde *x, t_signal **sp){
	//(freq*tablelen)/(samplerate) = array values per sample to advance
	// divide by tablelen to map to 0 to 1 range,..freq/samplerate
	    dsp_add(dxkadsr_tilde_perform, 3, x, sp[0]->s_vec, sp[0]->s_n);
		int sr = sp[0]->s_sr;
		if(sr != x->x_sr){
			...
			x->x_sr = sr;
			dxkadsr_tilde_attack(x,attms);
			dxkadsr_tilde_release(x,relms);
			dxkadsr_tilde_decay(x,decms);
			dxkadsr_tilde_sustain(x,sus);
		};

}


void dxkadsr_tilde_setup(void){
	dxkadsr_tilde_class = class_new(gensym("dxkadsr~"), (t_newmethod)dxkadsr_tilde_new, 0,
			sizeof(t_dxkadsr_tilde), CLASS_DEFAULT, A_GIMME, 0);
	class_addmethod(dxkadsr_tilde_class, (t_method)dxkadsr_tilde_dsp, gensym("dsp"), A_CANT, 0);
	class_addmethod(dxkadsr_tilde_class, (t_method)dxkadsr_tilde_attack, gensym("attack"), A_FLOAT, 0);
	class_addmethod(dxkadsr_tilde_class, (t_method)dxkadsr_tilde_decay, gensym("decay"), A_FLOAT, 0);
	class_addmethod(dxkadsr_tilde_class, (t_method)dxkadsr_tilde_sustain, gensym("sustain"), A_FLOAT, 0);
	class_addmethod(dxkadsr_tilde_class, (t_method)dxkadsr_tilde_release, gensym("release"), A_FLOAT, 0);
	class_addmethod(dxkadsr_tilde_class, (t_method)dxkadsr_tilde_level, gensym("level"), A_FLOAT, 0);
	class_addfloat(dxkadsr_tilde_class, dxkadsr_tilde_float);
