...
static t_class *dxksnbnkr_tilde_class;

typedef struct _dxksnbnkr_tilde_tilde {
	t_object x_obj;
	...
	t_inlet *x_rlistlet;
	t_outlet *x_outlet; 
} t_dxksnbnkr_tilde_tilde;

static void dxksnbnkr_tilde_abs_to_rel(t_dxksnbnkr_tilde_tilde *x){
...
static double get_t60mult(t_dxksnbnkr_tilde_tilde *x, float ringms){
...
static void dxksnbnkr_tilde_setmode(t_dxksnbnkr_tilde_tilde*x, t_symbol *s){
...
static void dxksnbnkr_tilde_init(t_dxksnbnkr_tilde_tilde *x){
	int i;
...
static void dxksnbnkr_tilde_freqs(t_dxksnbnkr_tilde_tilde *x, t_symbol *s, int argc, t_atom *argv){
	int i;
	for(i=0; i<argc; i++){
		if(i<DXKSBMAX){
	...
	x->x_numsin = argc;
	dxksnbnkr_tilde_abs_to_rel(x);
}

static void dxksnbnkr_tilde_amps(t_dxksnbnkr_tilde_tilde *x, t_symbol *s, int argc, t_atom *argv){
	int i;
	for(i=0; i<argc; i++){
		if(i<DXKSBMAX){
...
static void dxksnbnkr_tilde_rings(t_dxksnbnkr_tilde_tilde *x, t_symbol *s, int argc, t_atom *argv){
	int i;
	for(i=0; i<argc; i++){
		if(i<DXKSBMAX){
...
static void *dxksnbnkr_tilde_new(t_symbol *s, int argc, t_atom *argv){
	int i;
	t_dxksnbnkr_tilde_tilde *x = (t_dxksnbnkr_tilde_tilde *)pd_new(dxksnbnkr_tilde_class);
	x->x_mode = DXKSBMODE;
			...
			if(argc >= 2){//should be at leas two args
				...
				if(strcmp(curarg->s_name, "-mode") == 0){
					if(arg2 != &s_){//if not num, else just ignore arg
						dxksnbnkr_tilde_setmode(x, arg2);
					}; 
					argc-=2;
					argv+=2;
				}
...
static double dxksnbnkr_tilde_getsin(t_dxksnbnkr_tilde_tilde *x, double phs){
...
static t_int *dxksnbnkr_tilde_perform(t_int *w){
	t_dxksnbnkr_tilde_tilde *x = (t_dxksnbnkr_tilde_tilde *)(w[1]);
	...
	while(n--){
		...
		if(freq > 0.f && numsin > 0){
			for(i=0; i<numsin; i++){
				...
				double curout = dxksnbnkr_tilde_getsin(x, phs);
				curout *= (curamp);
...
static void dxksnbnkr_tilde_dsp(t_dxksnbnkr_tilde_tilde *x, t_signal **sp){
	//(freq*tablelen)/(samplerate) = array values per sample to advance
	// divide by tablelen to map to 0 to 1 range,..freq/samplerate
		dsp_add(dxksnbnkr_tilde_perform, 5, x, sp[0]->s_vec, sp[1]->s_vec,  sp[2]->s_vec, sp[0]->s_n);
		int sr = sp[0]->s_sr;
		if(x->x_sr != sr){
...
static void *dxksnbnkr_tilde_free(t_dxksnbnkr_tilde_tilde *x){
...
void dxksnbnkr_tilde_setup(void){
	dxksnbnkr_tilde_class = class_new(gensym("dxksnbnkr~"), (t_newmethod)dxksnbnkr_tilde_new, 0,
			sizeof(t_dxksnbnkr_tilde_tilde), CLASS_DEFAULT, A_GIMME, 0);
   CLASS_MAINSIGNALIN(dxksnbnkr_tilde_class, t_dxksnbnkr_tilde_tilde, x_input);
	class_addmethod(dxksnbnkr_tilde_class, (t_method)dxksnbnkr_tilde_dsp, gensym("dsp"), A_CANT, 0);
   class_addmethod(dxksnbnkr_tilde_class, (t_method)dxksnbnkr_tilde_freqs, gensym("freqs"), A_GIMME, 0);
   class_addmethod(dxksnbnkr_tilde_class, (t_method)dxksnbnkr_tilde_amps, gensym("amps"), A_GIMME, 0);
   class_addmethod(dxksnbnkr_tilde_class, (t_method)dxksnbnkr_tilde_rings, gensym("rings"), A_GIMME, 0);
   class_addmethod(dxksnbnkr_tilde_class, (t_method)dxksnbnkr_tilde_setmode, gensym("mode"), A_SYMBOL, 0);
