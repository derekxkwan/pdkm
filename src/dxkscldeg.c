...
static t_class *dxkscldeg_class;

typedef struct _dxkscldeg {
	t_object x_obj;
	char scalename[10];
	int curscale[10];
	t_float offset;
...
static void dxkscldeg_float(t_dxkscldeg *x, t_float f){
	int returnval;
	int iptoctave;
	int input = (int) f;
	int curscale[10];
...
static void dxkscldeg_pickscale(t_dxkscldeg *x, t_symbol *s){
...
static void *dxkscldeg_new(t_symbol *s, t_floatarg f){
	t_dxkscldeg *x = (t_dxkscldeg *)pd_new(dxkscldeg_class);
	dxkscldeg_pickscale(x, s);
	x->offset = f;
...
void dxkscldeg_setup(void){
	dxkscldeg_class = class_new(gensym("dxkscldeg"), (t_newmethod)dxkscldeg_new, 0,
			sizeof(t_dxkscldeg), 0, A_DEFSYMBOL, A_DEFFLOAT, 0);
	class_addfloat(dxkscldeg_class, (t_method)dxkscldeg_float);
	class_addmethod(dxkscldeg_class, (t_method)dxkscldeg_pickscale, gensym("scale"), A_SYMBOL, 0);
}
