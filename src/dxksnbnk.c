...
static t_class *dxksnbnk_tilde_class;

typedef struct _dxksnbnk_tilde {
	t_object x_obj;
...
static void dxksnbnk_tilde_abs_to_rel(t_dxksnbnk_tilde *x){
...
static void dxksnbnk_tilde_setmode(t_dxksnbnk_tilde*x, t_symbol *s){
...
static void dxksnbnk_tilde_init(t_dxksnbnk_tilde *x){
	int i;
...
static void dxksnbnk_tilde_freqs(t_dxksnbnk_tilde *x, t_symbol *s, int argc, t_atom *argv){
	int i;
	for(i=0; i<argc; i++){
		if(i<DXKSBMAX){
	...
	x->x_numsin = argc;
	dxksnbnk_tilde_abs_to_rel(x);
}

static void dxksnbnk_tilde_amps(t_dxksnbnk_tilde *x, t_symbol *s, int argc, t_atom *argv){
	int i;
	for(i=0; i<argc; i++){
		if(i<DXKSBMAX){
...
static void *dxksnbnk_tilde_new(t_symbol *s, int argc, t_atom *argv){
	int i;
	t_dxksnbnk_tilde *x = (t_dxksnbnk_tilde *)pd_new(dxksnbnk_tilde_class);
	x->x_mode = DXKSBMODE;
...
static double dxksnbnk_tilde_getsin(t_dxksnbnk_tilde *x, double phs){
...
static t_int *dxksnbnk_tilde_perform(t_int *w){
	
	t_dxksnbnk_tilde *x = (t_dxksnbnk_tilde *)(w[1]);
	...
	while(n--){
		...
		if(freq > 0.f && numsin > 0){
			for(i=0; i<numsin; i++){
				...
				double curout = dxksnbnk_tilde_getsin(x, phs);
				curout *= (curamp);
...
static void dxksnbnk_tilde_dsp(t_dxksnbnk_tilde *x, t_signal **sp){
	//(freq*tablelen)/(samplerate) = array values per sample to advance
	// divide by tablelen to map to 0 to 1 range,..freq/samplerate
	    dsp_add(dxksnbnk_tilde_perform, 4, x, sp[0]->s_vec, sp[1]->s_vec, sp[0]->s_n);
    x->x_conv = (double)DXKTABLEN/(double)sp[0]->s_sr; //amount to change phase for freq 1

}


static void dxksnbnk_tilde_ft1(t_dxksnbnk_tilde *x, t_symbol * s,int argc, t_atom *argv){//0 to 1
	int i;
...
static void *dxksnbnk_tilde_free(t_dxksnbnk_tilde *x){
...
void dxksnbnk_tilde_setup(void){
	dxksnbnk_tilde_class = class_new(gensym("dxksnbnk~"), (t_newmethod)dxksnbnk_tilde_new, 0,
			sizeof(t_dxksnbnk_tilde), CLASS_DEFAULT, A_GIMME, 0);
   CLASS_MAINSIGNALIN(dxksnbnk_tilde_class, t_dxksnbnk_tilde, x_freq);
	class_addmethod(dxksnbnk_tilde_class, (t_method)dxksnbnk_tilde_dsp, gensym("dsp"), A_CANT, 0);
   class_addmethod(dxksnbnk_tilde_class, (t_method)dxksnbnk_tilde_freqs, gensym("freqs"), A_GIMME, 0);
   class_addmethod(dxksnbnk_tilde_class, (t_method)dxksnbnk_tilde_amps, gensym("amps"), A_GIMME, 0);
   class_addmethod(dxksnbnk_tilde_class, (t_method)dxksnbnk_tilde_setmode, gensym("mode"), A_SYMBOL, 0);
	class_addmethod(dxksnbnk_tilde_class, (t_method)dxksnbnk_tilde_ft1, gensym("ft1"), A_GIMME, 0);
}
