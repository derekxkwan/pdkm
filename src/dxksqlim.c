...
static t_class *dxksqlim_tilde_class;

typedef struct _dxksqlim_tilde {
	t_object x_obj;
...
static void *dxksqlim_tilde_new(t_floatarg freq){
	int i,j;
	t_dxksqlim_tilde *x = (t_dxksqlim_tilde *)pd_new(dxksqlim_tilde_class);
	x->x_freq = freq;
	x->x_phase = 0.f;
...
static t_int *dxksqlim_tilde_perform(t_int *w){
	 t_dxksqlim_tilde *x = (t_dxksqlim_tilde *)(w[1]);
...
static void dxksqlim_tilde_dsp(t_dxksqlim_tilde *x, t_signal **sp){
    ...
    x->x_conv = (double)DXKTABLEN/(double)sp[0]->s_sr; //amount to change phase for freq 1
	    dsp_add(dxksqlim_tilde_perform, 4, x, sp[0]->s_vec, sp[1]->s_vec, sp[0]->s_n);

}

static void dxksqlim_tilde_ft1(t_dxksqlim_tilde *x, t_float phase){
	x->x_phase = (double)DXKTABLEN*phase;

}

static void *dxksqlim_tilde_free(t_dxksqlim_tilde *x){
...
void dxksqlim_tilde_setup(void){
	dxksqlim_tilde_class = class_new(gensym("dxksqlim~"), (t_newmethod)dxksqlim_tilde_new, 0,
			sizeof(t_dxksqlim_tilde), CLASS_DEFAULT, A_DEFFLOAT, 0);
	class_addmethod(dxksqlim_tilde_class, (t_method)dxksqlim_tilde_dsp, gensym("dsp"), A_CANT, 0);
   CLASS_MAINSIGNALIN(dxksqlim_tilde_class, t_dxksqlim_tilde, x_freq);
   class_addmethod(dxksqlim_tilde_class, (t_method)dxksqlim_tilde_ft1, gensym("ft1"), A_FLOAT, 0);
}
